...
class MobileNetV2_normal(object):
    ...
    def _build_model(self, num_to_reduce):
        ...
        self.output = tc.layers.conv2d(self.output, num_to_reduce*26, 1, normalizer_fn=self.normalizer, normalizer_params=self.bn_params)
    ...
    def head_nework(self, input, n_sigmoid, n_exp, num_to_reduce):
        with tf.variable_scope('head_network'):
            if self.is_training:
                ...
                keypoint_xy_class_probability = tc.layers.fully_connected(input_flat,800, activation_fn=tf.sigmoid)
                keypoint_xy_class_regression = tc.layers.fully_connected(keypoint_xy_class_probability,1600, activation_fn= None)
                classify = tf.reshape(keypoint_xy_class_probability, [-1, 10, 10, 8])
                reg = tf.reshape(keypoint_xy_class_regression, [-1, 10, 10, 16])

                keypoint_xy_class_probability_t = tf.concat([tf.reshape(reg[:,:,:,0],[-1,10,10,1]),tf.reshape(reg[:,:,:,1],[-1,10,10,1]), tf.reshape(classify[:,:,:,0],[-1,10,10,1]),
                                                             tf.reshape(reg[:,:,:,2],[-1,10,10,1]),tf.reshape(reg[:,:,:,3],[-1,10,10,1]), tf.reshape(classify[:,:,:,1],[-1,10,10,1]),
                                                             tf.reshape(reg[:,:,:,4],[-1,10,10,1]),tf.reshape(reg[:,:,:,5],[-1,10,10,1]), tf.reshape(classify[:,:,:,2],[-1,10,10,1]),
                                                             tf.reshape(reg[:,:,:,6],[-1,10,10,1]),tf.reshape(reg[:,:,:,7],[-1,10,10,1]), tf.reshape(classify[:,:,:,3],[-1,10,10,1]),
                                                             tf.reshape(reg[:,:,:,8],[-1,10,10,1]),tf.reshape(reg[:,:,:,9],[-1,10,10,1]), tf.reshape(classify[:,:,:,4],[-1,10,10,1]),
                                                             tf.reshape(reg[:,:,:,10],[-1,10,10,1]),tf.reshape(reg[:,:,:,11],[-1,10,10,1]), tf.reshape(classify[:,:,:,5],[-1,10,10,1]),
                                                             tf.reshape(reg[:,:,:,12],[-1,10,10,1]),tf.reshape(reg[:,:,:,13],[-1,10,10,1]), tf.reshape(classify[:,:,:,6],[-1,10,10,1]),
                                                             tf.reshape(classify[:,:,:,7],[-1,10,10,1]), tf.reshape(reg[:,:,:,14],[-1,10,10,1]),tf.reshape(reg[:,:,:,15],[-1,10,10,1])], 3)

                box_wh = tc.layers.fully_connected(input_flat, 200, activation_fn=None)
                box_wh = tf.reshape(box_wh, [-1, 10, 10, 2])

                output = tf.concat([keypoint_xy_class_probability_t, box_wh], 3)
