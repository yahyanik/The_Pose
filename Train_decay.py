...
def CPU_reminder_of_cost(wpar, y_true, batch_size):
    ...
    c1 = tf.keras.backend.sum(wpar[0], axis=-1)
    c2 = tf.keras.backend.sum(wpar[1], axis=-1)
    c3 = tf.keras.backend.sum(wpar[2], axis=-1)
    c4 = tf.keras.backend.sum(wpar[3], axis=-1)
    c5 = tf.keras.backend.sum(wpar[4], axis=-1)
    c6 = tf.keras.backend.sum(wpar[5], axis=-1)
    c7 = tf.keras.backend.sum(wpar[6], axis=-1)
    ...
    v1 = tf.keras.backend.sum(wpar[7], axis=-1)
    v2 = tf.keras.backend.sum(wpar[8], axis=-1)
    v3 = tf.keras.backend.sum(wpar[9], axis=-1)
    v4 = tf.keras.backend.sum(wpar[10], axis=-1)
    v5 = tf.keras.backend.sum(wpar[11], axis=-1)
    v6 = tf.keras.backend.sum(wpar[12], axis=-1)
    v7 = tf.keras.backend.sum(wpar[13], axis=-1)
    v8 = tf.keras.backend.sum(wpar[14], axis=-1)
    # confidence = ((v1 + v2 + v3 + v4 + v5 + v6 + v7 + v8) * lada) / 8

    v11 = tf.keras.backend.sum(wpar[15], axis=-1)
    v21 = tf.keras.backend.sum(wpar[16], axis=-1)
    v31 = tf.keras.backend.sum(wpar[17], axis=-1)
    v41 = tf.keras.backend.sum(wpar[18], axis=-1)
    v51 = tf.keras.backend.sum(wpar[19], axis=-1)
    v61 = tf.keras.backend.sum(wpar[20], axis=-1)
    v71 = tf.keras.backend.sum(wpar[21], axis=-1)
    v81 = tf.keras.backend.sum(wpar[22], axis=-1)
    ...
    b1 = tf.keras.backend.sum(wpar[23], axis=-1)
    b2 = tf.keras.backend.sum(wpar[24], axis=-1)
...
def cost (y_pred, y_true):
    ...
    c1 = tf.sign(y[:,2]) * (
            tf.keras.backend.square(y[:,0] - y_hat[:,0]) +
            tf.keras.backend.square(y[:,1] - y_hat[:,1]))

    c2 = tf.sign(y[:, 5]) * (
            tf.keras.backend.square(y[:, 3] - y_hat[:, 3]) +
            tf.keras.backend.square(y[:, 4] - y_hat[:, 4]))

    c3 = tf.sign(y[:, 8]) * (
            tf.keras.backend.square(y[:, 6] - y_hat[:, 6]) +
            tf.keras.backend.square(y[:, 7] - y_hat[:, 7]))

    c4 = tf.sign(y[:, 11]) * (
            tf.keras.backend.square(y[:, 10] - y_hat[:, 10]) +
            tf.keras.backend.square(y[:, 9] - y_hat[:, 9]))

    c5 = tf.sign(y[:, 14]) * (
            tf.keras.backend.square(y[:, 12] - y_hat[:, 12]) +
            tf.keras.backend.square(y[:, 13] - y_hat[:, 13]))

    c6 = tf.sign(y[:, 17]) * (
            tf.keras.backend.square(y[:, 15] - y_hat[:, 15]) +
            tf.keras.backend.square(y[:, 16] - y_hat[:, 16]))

    c7 = tf.sign(y[:, 20]) * (
            tf.keras.backend.square(y[:, 18] - y_hat[:, 18]) +
            tf.keras.backend.square(y[:, 19] - y_hat[:, 19]))
    ...
    v1 = ((-1) * tf.sign(y[:, 2]) + 1) * (tf.keras.backend.square(y_hat[:, 2] - tf.sign(y[:, 2])))
    v2 = ((-1) * tf.sign(y[:, 5]) + 1) * (tf.keras.backend.square(y_hat[:, 5] - tf.sign(y[:, 5])))
    v3 = ((-1) * tf.sign(y[:, 8]) + 1) * (tf.keras.backend.square(y_hat[:, 8] - tf.sign(y[:, 8])))
    v4 = ((-1) * tf.sign(y[:, 11]) + 1) * (tf.keras.backend.square(y_hat[:, 11] - tf.sign(y[:, 11])))
    v5 = ((-1) * tf.sign(y[:, 14]) + 1) * (tf.keras.backend.square(y_hat[:, 14] - tf.sign(y[:, 14])))
    v6 = ((-1) * tf.sign(y[:, 17]) + 1) * (tf.keras.backend.square(y_hat[:, 17] - tf.sign(y[:, 17])))
    v7 = ((-1) * tf.sign(y[:, 20]) + 1) * (tf.keras.backend.square(y_hat[:, 20] - tf.sign(y[:, 20])))
    v8 = ((-1) * tf.sign(y[:, 21]) + 1) * (tf.keras.backend.square(y_hat[:, 21] - tf.sign(y[:, 21])))

    # # confidence = ((v1 + v2 + v3 + v4 + v5 + v6 + v7 + v8) * lada)/8

    v11 = tf.sign(y[:, 2]) *(tf.keras.backend.square(y_hat[:, 2] - tf.sign(y[:, 2])))       # in model_1280relu_7 series there is no sign() for the
    v21 = tf.sign(y[:, 5]) *(tf.keras.backend.square(y_hat[:, 5] - tf.sign(y[:, 5])))   # y_hat in the error calculation just one for before the error
    v31 = tf.sign(y[:, 8]) *(tf.keras.backend.square(y_hat[:, 8] - tf.sign(y[:, 8])))
    v41 = tf.sign(y[:, 11]) *(tf.keras.backend.square(y_hat[:, 11] - tf.sign(y[:, 11])))
    v51 = tf.sign(y[:, 14]) *(tf.keras.backend.square(y_hat[:, 14] - tf.sign(y[:, 14])))
    v61 = tf.sign(y[:, 17]) *(tf.keras.backend.square(y_hat[:, 17] - tf.sign(y[:, 17])))
    v71 = tf.sign(y[:, 20]) *(tf.keras.backend.square(y_hat[:, 20] - tf.sign(y[:, 20])))
    v81 = tf.sign(y[:, 21]) *(tf.keras.backend.square(y_hat[:, 21] - tf.sign(y[:, 21])))

    b1 = tf.sign(y[:, 21]) * (
            tf.keras.backend.square(y[:, 22] - y_hat[:, 22]) +
            tf.keras.backend.square(y[:, 23] - y_hat[:, 23]))

    b2 = tf.sign(y[:, 21]) * (
            tf.keras.backend.square(tf.keras.backend.sqrt(y[:, 24]) - tf.keras.backend.sqrt(y_hat[:, 24])) +
            tf.keras.backend.square(tf.keras.backend.sqrt(y[:, 25]) - tf.keras.backend.sqrt(y_hat[:, 25])))
...
def train (imagesize = 320, batch_size = 32, num_threads = 10, epoch = 1000, learning_rate = 0.001, num_gpus = 2):

    FolderName = './model_normal_2.2.32_decay'
    ...
    tf.summary.scalar('PERCISION scoree', percision)
    ...
    with tf.Session(config=config) as sess:
        ...
        for epoch in range(hm_epochs):
            ...
            for bat in range(num_batch):
                ...
                img, anno = sess.run([image, annotation])
                summary, _, c, _, _, _, accur, pre = sess.run([merged, optimizer,
                                                                loss, f1, f2, f3, accuracy, model.output],
                                                                feed_dict={x: img, y_tr: anno})
                epoch_loss += np.sum(c)
                epoch_accu += accur
                # print pre[0,:,:,21]
                re_pre = np.reshape(pre, (-1, 26))
                ...
                if (bat) % 100 == 0:
                    ...
                    re_pre = np.reshape(pre, (-1, 26))
                    re_tr = np.reshape(anno, (-1, 26))
                    print 'annotation (', np.min(re_tr[:, :24]), ',', np.max(re_tr[:, :24]), ',', np.min(
                        re_tr[:, 24:]), ',', np.max(re_tr[:, 24:]), ')', \
                        'prediction (', np.min(re_pre[:, :24]), ',', np.max(re_pre[:, :24]), ',', np.min(
            ...
            if epoch % 10 == 0:
                save_path = saver.save(sess, FolderName+"/tmp/model.ckpt", global_step=epoch + 2)
