...
def test_mAP (epoch = 120, layers_not_training =117, learning_rate = 0.001, drop_out = 0.7, regular_fac = 0.05, num_to_reduce=32, imagesize = 320, batch_size = 32, num_threads = 10, num_gpus = 2):


    FolderName = './normal{}_94.{}_{}_{}_0.5_5'.format(batch_size, learning_rate, layers_not_training,regular_fac)
    ...
    with tf.Session() as sess:
        ...
        for bat_v in range(1):
            ...
            # res = model_obj.model.predict(img, batch_size=1, verbose=1, steps=500)
            # print(res)
            # print (np.max(res))
            # print(res.shape)
            # print(np.min(res))
            ...
            print(tok-tik)
    # print(res)
    # loss, acc = parallel_model.evaluate(image_val, annotation_val, batch_size=batch_size, steps=num_batch_val, verbose=1, callbacks=[tensorboard])320
