...
class MobileNetV2_1280relu_new(object):
    ...
    def _build_model(self, num_to_reduce):
        ...
        self.output = self._inverted_bottleneck(output, 1, 4, 0)
        self.output = self._inverted_bottleneck(self.output, 6, 6, 1)  # 6 is the t and 34 is the output shape(finlter number and layers are repeated in for the n in the paper
        self.output = self._inverted_bottleneck(self.output, 6, 6, 0)
        self.output = self._inverted_bottleneck(self.output, 6, 8, 1)
        self.output = self._inverted_bottleneck(self.output, 6, 8, 0)
        self.output = self._inverted_bottleneck(self.output, 6, 8, 0)
        ...
        self.output = self._inverted_bottleneck(self.output, 6, 24, 0)
        self.output = self._inverted_bottleneck(self.output, 6, 24, 0)
        self.output = self._inverted_bottleneck(self.output, 6, 40, 1)
        self.output = self._inverted_bottleneck(self.output, 6, 40, 0)
        self.output = self._inverted_bottleneck(self.output, 6, 40, 0)
